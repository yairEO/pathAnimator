...
(function(){
	...
	function generateWalker(walkerObj, color) {
		//function PathAnimator (path, target, durationSecs, loop, reverse, easingFn) 
		var newAnimatedWalker = new PathAnimator(path, walkerObj, 30, true);
		walkerObj.firstChild.style.color = color;
	...
	generateWalker(firstWalkerObj, firstColor).start();
	// bind the first Controller to the first Walker
	var firstController = $('menu > div:first');
	firstController[0].style.borderColor = firstColor;
...
------------------------------------------------------------*/
	...
	function addWalker(){
		var newWalker = firstWalkerObj.cloneNode(true),
			...
			color = randomColor(),
			newAnimatedWalker = generateWalker(newWalker, color);
	...
	function stopPlay(){
		$(this.parentNode.parentNode).data('walker').pauseResume();
	}
	
	// switch direction of the animated object 
	function switchDirection(){
		var walker = $(this.parentNode.parentNode).data('walker');
		walker.reverse = !walker.reverse;
	}

	function changeSpeed(){
		var walker = $(this.parentNode).data('walker');
		walker.startTime = null;
		walker.duration = Number(this.value) * 1000;
	...
	function removeInstance(){
		...
		if( walkers.length > 1 ){
			parent.remove();
			thisAnimatedWalker.stop();
			$(thisAnimatedWalker.d).remove();
	...
	function changeEasing(){
		...
		thisAnimatedWalker.easingFn = easingFunc;
	}
