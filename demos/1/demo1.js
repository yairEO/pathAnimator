...
------------------------------------------------------------*/
	...
	$('menu')
		...
		.on('change', '.stopPlay input', stopPlay)
		.on('change', '.reverse input', switchDirection)
	...
	function switchDirection(){
		var thisAnimatedWalker = $(this.parentNode.parentNode).data('walker'),
			settings = {
				reverse : !thisAnimatedWalker.pathAnimator.settings.reverse
			};

		if( thisAnimatedWalker.pathAnimator.running )
			thisAnimatedWalker.resume.call(thisAnimatedWalker, settings);
	}
	...
	function changeEasing(){
		...
		if( this.value ){
			thisAnimatedWalker.resume.call(thisAnimatedWalker, {
				easing : function(t){ return eval(formula) }
			});
		}
	}
